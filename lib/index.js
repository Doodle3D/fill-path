...
export default function(paths, point) {
  ...
  clipperOffset.AddPaths(paths, ClipperLib.JoinType.jtSquare, ClipperLib.EndType.etOpenSquare);
  ...
  outLines.sort((a, b) => Math.abs(ClipperLib.Clipper.Area(a)) > Math.abs(ClipperLib.Clipper.Area(b)));
  ...
  fillShapeLoop: for (const fillShape of fillShapes) {
    // loop through paths of fill shape
    for (let i = 0; i < fillShape.length; i ++) {
      ...
      if ((isOutline && !pointInPolygon) || (!isOutline && pointInPolygon)) {
